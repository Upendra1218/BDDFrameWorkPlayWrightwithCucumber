@ETG @PlaceOrderWithRedeemGiftVoucher
Feature: Purchase Product from B2B Store with Redeeming Gift Certificate

  @PlaceOrderWithRedeemGiftVoucher
  Scenario Outline: Purchase Product from B2B Store with Redeeming Gift Certificate
    ...
    And User enters <GiftCertificate> in the "Redeem Gift Certificate" field on "Payment" Page
    When User clicks on the "Apply" button on "Payment" Page
    Then Verify "Gift Certificate Redeemed Successfully" message on "Payment" Page
    When User enters all the "Credit Card Details" on "Payment" Page
    ...
    And Verify "Gift Certificate Amount" is already "Captured" on "Order Summary" record
    ...
    @PlaceOrderWithRedeemGiftVoucher
    Examples: 
      | SubCategory | Category | Product              | Color  | Size | Quantity | Email                           | GiftCertificate    |
      | "Pants"     | "Men"    | "Straight Leg Pants" | "Gray" | "32" | "4"      | "yatharthdubeycorp01@gmail.com" | "RRKFIWSAVTHFGAQC" |
